...
export class CasparServerRepository {
    ...
    async findAll(): Promise<DeviceConfig[]> {
        ...
        const servers = await this.db.query<any[]>('SELECT * FROM casparcg_servers ORDER BY name');
    ...
    async findById(id: number): Promise<DeviceConfig | null> {
        ...
        const servers = await this.db.query<any[]>(
            'SELECT * FROM casparcg_servers WHERE id = ?',
            [id]
        );
        return servers[0] ? this.mapToDeviceConfig(servers[0]) : null;
    }
